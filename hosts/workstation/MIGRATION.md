...
| Notification daemon | **mako** — reversed 2026-08-02 from "keep `dunst`", which had been recorded as a choice with no rationale attached. Decided on the tools' merits: mako is Wayland-native (wlr-layer-shell) on a Wayland-only host, its named composable *modes* are a better do-not-disturb primitive than dunst's single pause axis, and `makoctl menu` delegates action-picking to wofi, which this desktop already uses. Accepted cost, eyes open: mako's `restore` only pops the *most recent* notification — dunst's `history-pop [ID]`/`history-rm` have no equivalent — and there is no `dunstify`, so scripted notifications are plain `notify-send`. If either becomes load-bearing, that is the trigger to revisit. |
| Login shell | zsh |
...
**Resolved 2026-08-02: switched to mako**, see the decision table above. Worth
recording what "will start with no config" actually meant, because it was worse
than it sounds: dunst ships `org.knopwob.dunst.service` claiming
`org.freedesktop.Notifications`, so it D-Bus activates on the first
notification whether or not anything launches it — which is why nothing in the
hypr config execs a notification daemon, and why the mismatch would never have
surfaced as an error. It would simply have run with stock styling forever.
